...
# --- 1. 数据库配置 (升级为 v7 增加纤维字段) ---
conn = sqlite3.connect('nutrition_tracker_v7.db', check_same_thread=False)
c = conn.cursor()

c.execute('''CREATE TABLE IF NOT EXISTS food_lib 
             (name TEXT PRIMARY KEY, cal_per_g REAL, pro_per_g REAL, fat_per_g REAL, carb_per_g REAL, fiber_per_g REAL)''')
c.execute('''CREATE TABLE IF NOT EXISTS daily_log 
             (id INTEGER PRIMARY KEY AUTOINCREMENT, date TEXT, name TEXT, weight REAL, 
...
st.markdown(
    ...
    [data-testid="stMetricValue"] {
        font-size: 22px !important; /* 你可以根据喜好把 22 改成 20 或 24 */
    }
    </style>
    """,
...
if menu == "饮食记录与今日概览":
    ...
    with col1:
        ...
        c.execute("SELECT name FROM food_lib")
        ...
        if selected_option == "➕ 手动录入新食物...":
            food_name = st.text_input("请输入新食物名称", placeholder="例如：燕麦片")
        else:
            food_name = selected_option

        input_weight = st.number_input("摄入重量 (g)", min_value=0.1, value=100.0)

        if food_name:
            c.execute("SELECT * FROM food_lib WHERE name=?", (food_name,))
            f_data = c.fetchone()
            ...
            if not f_data:
                ...
                u1, u2 = st.columns(2)
                with u1:
                    base_w = st.number_input("基准重量 (g)", value=100.0)
                ...
                with u2:
                    ...
                    r_fiber = st.number_input(f"膳食纤维 (g)", 0.0)  # 新增膳食纤维输入
                ...
                st.success(f"✅ 已选中食物库：【{food_name}】")
                cal_pg, pro_pg, fat_pg, carb_pg, fiber_pg = f_data[1], f_data[2], f_data[3], f_data[4], f_data[5]

            if st.button("确认添加记录", use_container_width=True):
                if not f_data and food_name:
                    c.execute("INSERT INTO food_lib VALUES (?,?,?,?,?,?)",
                              (food_name, cal_pg, pro_pg, fat_pg, carb_pg, fiber_pg))
                now_date = datetime.now().strftime("%Y-%m-%d")
                c.execute(
                    "INSERT INTO daily_log (date, name, weight, cal, pro, fat, carb, fiber) VALUES (?,?,?,?,?,?,?,?)",
                    (now_date, food_name, input_weight, cal_pg * input_weight, pro_pg * input_weight,
                     fat_pg * input_weight, carb_pg * input_weight, fiber_pg * input_weight))
                conn.commit()
                st.rerun()
    ...
    with col2:
        ...
        today_df = pd.read_sql_query(f"SELECT * FROM daily_log WHERE date='{today}'", conn)

        if profile:
            target_cal, target_pro, target_fat, target_carb, target_fiber = profile[6], profile[7], profile[8], profile[
                9], profile[10]
        else:
        ...
        if not today_df.empty:
            ...
            m5.metric("膳食纤维", f"{totals['fiber']:.1f} / {target_fiber:.0f} g")
            ...
            pro_energy = totals['pro'] * 4
            fat_energy = totals['fat'] * 9
            carb_energy = totals['carb'] * 4
            ...
            fig = px.pie(values=[pro_energy, fat_energy, carb_energy], names=['蛋白质供能', '脂肪供能', '碳水供能'],
                         hole=0.4)
    ...
    with tab_list:
        if not today_df.empty:
            for _, row in today_df.iterrows():
                ...
                cols[1].write(f"{row['weight']}g")
                cols[2].write(f"{row['cal']:.1f} kcal")
                cols[3].write(f"蛋白: {row['pro']:.1f}g")
                cols[4].write(f"脂肪: {row['fat']:.1f}g")
                cols[5].write(f"碳水: {row['carb']:.1f}g")
                cols[6].write(f"纤维: {row['fiber']:.1f}g")
                if cols[7].button("🗑️", key=f"del_{row['id']}"):
                    c.execute("DELETE FROM daily_log WHERE id=?", (row['id'],))
                    conn.commit()
                    st.rerun()
    with tab_history:
        all_log = pd.read_sql_query("SELECT * FROM daily_log", conn)
        if not all_log.empty:
            st.plotly_chart(px.bar(all_log.groupby('date').sum(numeric_only=True).reset_index(), x='date', y='cal',
                                   title="热量波动趋势"), use_container_width=True)
...
# --- 4. 功能模块：个人目标设置 ---
    ...
    with st.form("profile_form"):
        ...
        activity = st.selectbox("日常活动水平", list(activities.keys()),
                                index=list(activities.keys()).index(def_activity))
        ...
        if st.form_submit_button("计算并保存我的营养目标", type="primary"):
            ...
            t_fat = (tdee * 0.30) / 9
            t_carb = (tdee * 0.50) / 4
            t_fiber = (tdee / 1000) * 14  # 纤维推荐标准：14g / 1000kcal

            c.execute('''INSERT OR REPLACE INTO user_profile 
                         (id, gender, age, height, weight, activity, target_cal, target_pro, target_fat, target_carb, target_fiber) 
                         VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                      (gender, age, height, weight, activity, tdee, t_pro, t_fat, t_carb, t_fiber))
            conn.commit()
            st.balloons()
            st.success("目标已计算并成功保存！")
            st.rerun()
    ...
    if profile:
        ...
        st.info("💡 提示：膳食纤维目标按每 1000 kcal 热量匹配 14g 的健康标准为您计算。")

# --- 5. 食物库管理 ---
elif menu == "食物库管理(修改/查看)":
    st.header("📦 食物库管理")
    lib_df = pd.read_sql_query("SELECT * FROM food_lib", conn)
    if not lib_df.empty:
        ...
        if st.button("保存修改"):
            ...
            c.execute("DELETE FROM food_lib")
            final_save.to_sql('food_lib', conn, if_exists='append', index=False)
            conn.commit()
            st.success("更新成功！")
            st.rerun()
